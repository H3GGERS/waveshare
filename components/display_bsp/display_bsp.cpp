      ...
      width_(width), height_(height)
{
    ...
    io_config.pclk_hz           = 10 * 1000 * 1000;  // 10 MHz (match official driver)
    ...
    DisplayLen = transfer >> 3;  // 1 bit per pixel (match official Waveshare ESP32-S3-RLCD-4.2)
...
void DisplayPort::RLCD_Init()
{
    ...
    RLCD_SendData(0x11);  // 2-bit per pixel
    ...
    RLCD_Display();  // Push white frame to confirm init is working
}
